...
const CHAPTER_PREFIX = /^第\s*\d+\s*章\s*/u;
...
export function normalizeFanqieBinding(raw, context = 'fanqie') {
  ...
  return {
    ...
    timeZone,
    quality,
    schedule: { firstChapter, firstDate, chaptersPerDay, time },
  };
}
...
export function calculateFanqiePublishAt(chapterNumber, schedule) {
  ...
  const dayOffset = Math.floor((chapter - schedule.firstChapter) / schedule.chaptersPerDay);
  const date = new Date(`${schedule.firstDate}T00:00:00Z`);
  date.setUTCDate(date.getUTCDate() + dayOffset);
  return { date: date.toISOString().slice(0, 10), time: schedule.time };
}
...
export function createFanqieUploadPlan(localChapters, remoteChapters, options = {}) {
  ...
  const nextChapter = remoteChapters.length + 1;
  ...
  if (requestedTo < requestedFrom) throw new Error('--to 不能小于 --from');
