...
const NON_CHAPTER_BODY_ASSETS = new Set(['番茄书籍信息.md']);
...
export function normalizeFanqieBinding(raw, context = 'fanqie') {
  ...
  if (raw.contestParticipation != null && typeof raw.contestParticipation !== 'boolean') {
    throw new Error(`${context}.contestParticipation 必须是 true 或 false`);
  }
  ...
  if (sourceDir !== '正文') {
    throw new Error(`${context}.sourceDir 必须固定为“正文”，禁止从原文、拆分或其他目录发布`);
  }
  ...
  return {
    ...
    workTitle,
    sourceDir,
    contestParticipation: raw.contestParticipation !== false,
    aiUsed: raw.aiUsed,
    contentDetection,
    timeZone,
    quality,
...
async function chapterFilesIn(dir) {
  ...
  return entries
    .filter((entry) => entry.isFile()
      && CHAPTER_EXTENSIONS.has(path.extname(entry.name).toLowerCase())
      && !NON_CHAPTER_BODY_ASSETS.has(entry.name))
