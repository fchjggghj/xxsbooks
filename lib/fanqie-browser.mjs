...
import { classifyFanqieMarketingSubmission, FANQIE_TAG_GROUPS } from './fanqie-marketing.mjs';
import { assertFanqiePageContract } from './fanqie-page-contract.mjs';

const DEFAULT_TIMEOUT = 25_000;
const SAFE_DISMISS_BUTTON = /^(?:我知道了|知道了|好的|下次再说|以后再说|暂不|跳过|关闭|完成)$/u;
...
export function fanqieBookInfoUrl(binding) {
  return `https://fanqienovel.com/main/writer/book-info/${binding.workId}?type=1`;
}

async function selectMarketingTags(page, entry) {
  const chips = page.locator('.select-view .create-category-item');
  while (await chips.count() > 1) {
    await chips.nth((await chips.count()) - 1).locator('.item-del').click();
    await page.waitForTimeout(70);
  }
  await page.locator('.select-view').click();
  const modal = page.locator('.category-modal:visible');
  await modal.waitFor({ state: 'visible' });
  for (const group of FANQIE_TAG_GROUPS) {
    await modal.getByText(group, { exact: true }).click();
    for (const tag of entry.tags[group]) {
      const item = modal
        .locator('.arco-tabs-content-item-active .category-choose-list .category-choose-item-title')
        .filter({ hasText: new RegExp(`^${tag}$`, 'u') });
      const count = await item.count();
      if (count !== 1) throw new Error(`番茄“${group}”中无法唯一定位标签“${tag}”（匹配 ${count} 个）`);
      await item.click();
      await page.waitForTimeout(70);
    }
  }
...
async function fillMarketingFields(page, entry) {
  await selectMarketingTags(page, entry);
  await page.getByPlaceholder('请输入主角名1').fill(entry.protagonists[0] || '');
  const second = page.getByPlaceholder('请输入主角名2');
  if (await second.count()) await second.fill(entry.protagonists[1] || '');
  await page.locator('textarea').first().fill(entry.intro);
  const selected = await page.locator('.select-view .item-title').allInnerTexts();
  const expected = [entry.mainCategory, ...FANQIE_TAG_GROUPS.flatMap((group) => entry.tags[group])];
  const missing = expected.filter((value) => !selected.includes(value));
  if (missing.length) throw new Error(`提交前标签校验失败，缺少：${missing.join('、')}`);
  if (await page.locator('textarea').first().inputValue() !== entry.intro) throw new Error('提交前简介校验失败');
}

async function uploadMarketingCover(page, entry) {
  await page.getByRole('button', { name: '选择封面', exact: true }).click();
  const visibleModal = page.locator('.arco-modal:visible').filter({ has: page.locator('input[type="file"]') }).last();
  const file = visibleModal.locator('input[type="file"]').last();
  await file.setInputFiles(entry.coverFile);
  const confirm = visibleModal.getByRole('button', { name: '确定', exact: true });
  await confirm.waitFor({ state: 'visible' });
  // Playwright 会在这个“可见的封面弹窗”内等待按钮启用。不要全局查找“确定”，
  // 因为作家后台会预加载隐藏的“确认完结”等模态框。
  await confirm.click({ timeout: 10_000 });
}

export async function updateFanqieBookMarketing(page, binding, entry, options = {}) {
  if (entry.workId !== binding.workId || entry.workTitle !== binding.workTitle || entry.accountRef !== binding.accountRef) {
    throw new Error('营销资料与书籍的账号、作品 ID 或后台书名不一致，已停止以避免改错作品');
  }
  await page.goto(fanqieBookInfoUrl(binding), { waitUntil: 'domcontentloaded' });
  await page.waitForFunction((workTitle) => (document.body?.innerText || '').includes(workTitle), binding.workTitle);
  const edit = page.getByText('修改', { exact: true });
  if (await edit.isDisabled().catch(() => false)) {
    return { ok: true, status: 'pending_review', applied: false, reason: 'edit_locked' };
  }
  await edit.click();
  await page.locator('textarea').first().waitFor({ state: 'visible' });
  await fillMarketingFields(page, entry);
  await uploadMarketingCover(page, entry);
  await options.onReady?.(page, entry);
  await page.getByRole('button', { name: '立即修改', exact: true }).click();
  await Promise.race([
    page.getByText('修改成功', { exact: false }).waitFor({ state: 'visible' }),
    page.waitForFunction(() => new URL(location.href).searchParams.get('isEdit') === '0'),
  ]).catch(() => {});
  await page.waitForTimeout(800);
  const body = await page.locator('body').innerText();
  const result = classifyFanqieMarketingSubmission({
    body,
    entry,
    editDisabled: await page.getByText('修改', { exact: true }).isDisabled().catch(() => false),
    successToast: body.includes('修改成功'),
  });
  if (!result.ok) throw new Error(`作品资料提交状态不确定，缺少字段：${result.missingFields.join('、') || '简介'}`);
