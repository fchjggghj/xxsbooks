...
export async function alignCampaignAccounts(projectRoot, options = {}) {
  const root = path.resolve(projectRoot);
  const saved = await loadCampaignState(root);
  if (!saved.state) throw new Error('投放尚未初始化，请先 bootstrap。');
  const pool = await accountPool(root);
  const available = new Set(pool.accounts.map((item) => item.ref));
  const targetRefs = new Set();
  const changes = [];
  const lanes = {};
  for (const [key, lane] of Object.entries(saved.state.lanes || {})) {
    let next = lane;
    if (lane.current?.configFile) {
      const configFile = resolveInside(root, lane.current.configFile);
      const config = await readJson(configFile);
      if (config.name !== lane.current.book) {
        throw new Error(`投放线 ${lane.lane} 的配置书名与当前书籍不一致，拒绝对齐账号。`);
      }
      const accountRef = String(config.fanqie?.accountRef || '').trim();
      if (accountRef) {
        if (!available.has(accountRef)) throw new Error(`投放线 ${lane.lane} 的书籍绑定账号未在可用账号池中: ${accountRef}`);
        if (targetRefs.has(accountRef)) throw new Error(`多条投放线的当前书籍重复绑定账号: ${accountRef}`);
        targetRefs.add(accountRef);
        if (lane.accountRef !== accountRef) {
          changes.push({ lane: lane.lane, book: lane.current.book, from: lane.accountRef, to: accountRef });
          next = { ...lane, accountRef };
        }
      }
    }
    lanes[key] = next;
  }
  const state = { ...saved.state, updatedAt: new Date().toISOString(), lanes };
  if (options.apply && changes.length) {
    await withCampaignWrite(root, 'campaign-align-accounts', () => writeJsonAtomic(saved.file, state));
  }
  return {
    ok: true,
    command: 'campaign align-accounts',
    applied: options.apply === true && changes.length > 0,
    readOnly: options.apply !== true,
