...
import { connectFanqieBrowser, inspectFanqieRemoteChapters, uploadFanqiePlan } from './fanqie-browser.mjs';
...
export async function getFanqieLocalStatus(projectRoot, requestedBook = '') {
  ...
  return { ok: assignments.ok && results.every((item) => !item.error && item.quality?.ok), mode: 'local', assignments, lock, books: results };
}
...
export async function inspectFanqieBook(projectRoot, options = {}) {
  ...
  try {
    ...
    return { loaded, connection, remote, plan, state, preflight };
  } catch (error) {
...
export function fanqieRemoteOverview(inspection, mode = 'preview') {
  const { loaded, remote, plan, state, preflight } = inspection;
  return {
    ok: preflight.ok,
    mode,
    book: loaded.book.name,
...
export async function runFanqieUpload(projectRoot, options = {}) {
  ...
  const bookName = await resolveFanqieBookName(projectRoot, options.book);
  const lock = await acquireFanqieLock(projectRoot, bookName);
  let inspection;
  try {
...
export async function runFanqieReconcile(projectRoot, options = {}) {
  const bookName = await resolveFanqieBookName(projectRoot, options.book);
  const lock = options.apply ? await acquireFanqieLock(projectRoot, bookName) : null;
  let inspection;
  try {
    inspection = await inspectFanqieBook(projectRoot, options);
