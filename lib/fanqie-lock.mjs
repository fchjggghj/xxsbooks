...
export async function acquireFanqieLock(projectRoot, bookName) {
  const stateDir = path.join(projectRoot, '书籍', '.state', 'fanqie');
  const lockFile = path.join(stateDir, '.publish.lock.json');
...
export async function inspectFanqieLock(projectRoot) {
  const lockFile = path.join(projectRoot, '书籍', '.state', 'fanqie', '.publish.lock.json');
  let info;
  try {
    info = JSON.parse(await fs.readFile(lockFile, 'utf8'));
  } catch (error) {
    if (error.code === 'ENOENT') return { path: lockFile, exists: false, active: false, info: null };
    return { path: lockFile, exists: true, active: null, info: null, error: error.message };
  }
  let active = false;
  try {
  ...
  return { path: lockFile, exists: true, active, stale: !active, info };
}
